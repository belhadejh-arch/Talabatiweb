...
export function AdminLayout({ children }: { children: ReactNode }) {
      ...
      {navItems.map((item) => {
        ...
        return (
          <Link key={item.path} href={item.path} className={cn(
            "flex items-center gap-3 px-3 sm:px-4 py-3 mx-3 sm:mx-4 rounded-xl text-sm font-medium transition-all duration-200",
            isActive 
  ...
  return (
    <div className="flex h-[100dvh] min-w-0 bg-background font-sans overflow-hidden">
      {/* Desktop Sidebar */}
      <aside className="hidden w-64 shrink-0 flex-col border-e border-sidebar-border bg-sidebar md:flex shadow-xl shadow-sidebar/5 z-10 relative">
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden relative">
        {/* Top Header */}
        <header className="flex min-h-16 items-center justify-between gap-3 px-3 py-3 sm:h-20 sm:px-8 bg-background/80 backdrop-blur-md border-b border-border/50 sticky top-0 z-20">
          <div className="flex min-w-0 items-center gap-3 sm:gap-4">
            <Sheet>
              ...
              <SheetContent side="right" className="w-[min(18rem,calc(100vw-1rem))] p-0 bg-sidebar border-sidebar-border text-sidebar-foreground">
            ...
            <p className="sm:hidden truncate text-base font-bold text-foreground">
              {navItems.find(i => location.startsWith(i.path))?.label || "لوحة القيادة"}
            </p>
          </div>
